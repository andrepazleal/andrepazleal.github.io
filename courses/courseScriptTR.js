...
function showPainelContent(){
	...
	for(i=0; i < mobileCanvasImage.length; i++){
       	//mobileCanvasImage[i].appendChild(mobileDivImages[i])
       	//mobileCanvasImage[i].insertBefore(mobileDivImages[i], mobileCanvasImage[i].firstChild)

		//mobileCanvasImage[i].addEventListener('click', mostraMensagemImagem, false);
		//mobileCanvasImage[i].addEventListener('click', mobileShowText, false);

	}
	for(i=0; i < mobileDivImages.length; i++){
		//mobileDivImages[i].children[0].className="blurIn"
		mobileDivImages[i].addEventListener('click', mobileShowText, false);
		clickMeIcon = document.createElement("span")
        mobileCanvasImage[i].appendChild(clickMeIcon);
        clickMeIcon.className="blink";  
        //mobileDivImages[i].insertBefore(mobileCanvasImage[i], mobileDivImages[i].firstChild)
        //mobileCanvasImage[i].appendChild(mobileDivImages[i]);
        
	}
	if(isIOs == true){
        for(i=0; i < mobileDivImages.length; i++){
        //console.log(mobileDivImages[i].attributes[4].nodeValue)	
        //var imageDescriptionMove =  mobileDivImages[i].children[0].attributes[4].nodeValue;
			var newas = document.createElement("p");
			newas.style.opacity=0;
			newas.innerHTML=mobileDivImages[i].attributes[4].nodeValue;
			mobileDivImages[i].appendChild(newas);
        }
        
		//this.className="closeTurnImage";
		//var imageDescriptionMoveBefore = document.querySelectorAll("div[id="+"re-"+this.id+"c"+"]");
	}
    ...
    for(i=0; i < motionText.length; i++){
        ...
        if(isIOs == true){
			console.log(motionText[i])
        //motionTextCanvas[i].appendChild(motionText[i]);
        	//motionText[i].className="motion";
        	var motionTextNew = document.createElement("p");
			motionTextNew.innerHTML=motionText[i].attributes[4].nodeValue;
			mobileDivImages[i].appendChild(motionTextNew);
			//motionText
        //var test = motionTextCanvas[i].childNodes[1];
        //motionTextCanvas[i].childNodes[1].className="motion";
    }
    ...
    if(document.documentElement.clientWidth < 420){
        for(i=0; i < mobileCanvasImage.length; i++){
			...
			//var logoCourseClone = mobileCanvasImage[i].cloneNode(true);
			//painelTextoSlide.appendChild(logoCourseClone);

			//painelTextoSlide.appendChild(mobileCanvasImage[i].firstChild)
			
			//painelTextoSlide.appendChild(mobileCanvasImage[i]);
		}
...
function mobileShowText(event){
	if(isIOs == true){
		//this.className="closeTurnImage";
		changeColors();
		var canvasMobile = document.querySelectorAll("div[id="+"re-"+this.id+"c"+"]");
		canvasMobile[0].className="blurAll";
		this.children[0].className="openTurnImage"
		//var imageDescriptionMoveBefore = this.id.replace('re-','').replace('c','');
		...
		canvasMobile[0].className="blurAll"
		this.children[0].className="openTurnImage"
		this.children[0].children[0].className="openTurnImage"	
	}
...
function courseName(){
	...
	if(courseName[0] == 'Agente' && courseName[2] == 'Backup')
	{	
		courseName = 'backupNuvem.png';
	}
	if(courseName[0] == 'Itens' && courseName[4] == 'Plus')
	{	
		courseName = 'contabilPlus10.png';
	}
	if(courseName[1] == 'Word')
	{	
		courseName = 'word.png';
	}
	if(courseName[1] == 'Excel')
	{	
		courseName = 'excel.png';
	}
	if(courseName[1] == 'PowerPoint')
	{	
		courseName = 'powerpoint.png';
	}
}
...
function introVideos(){
	...
	if(cpInfoCurrentSlideLabel == "Certification!"){
		createElements();
		certification();
	}
}
...
function certification(){
	...
	//setTimeout(beneficiosRemoveLogos, 2000);
	//function beneficiosRemoveLogos(){

		//console.log(welcomeIcon.style.opacity)
	try{
		welcomeIcon.className="scaleOut";
		logoCourse.className="scaleOut";	
	}catch(e){}
	//}
	changeColors();

	//$(painelTextoSlide).html(cpInfoCurrentSlideLabel+"<br>"+nomeAluno);

	painelTitleSlide.innerHTML="";
	painelTextoSlide.innerHTML="";
	painelNomeTopico.innerHTML="";
	$(painelTextoSlide).css('margin-left',"0");
	$(painelTextoSlide).css('max-width',"inherit");
	$(painelTextoSlide).css('max-height',"inherit");
	
	//logoCourse.parentNode.insertBefore(painel, logoCourse.nextSibling);
	//$(painelTextoSlide).append(logoCourse);
	...
	certification1.innerHTML='<span class="lnr lnr-sun"></span><br><br>'+certification1text;

	var certification2text = document.querySelectorAll('div[id*="Slide"] + div[id*="accStr"]')[0].children[0].innerHTML.replace('Certification!','').split('  ')[1]
	certification2.innerHTML='<span class="lnr lnr-graduation-hat"></span><br><br>'+certification2text;

	var certification3text = document.querySelectorAll('div[id*="Slide"] + div[id*="accStr"]')[0].children[0].innerHTML.replace('Certification!','').split('  ')[2]
	certification3.innerHTML='<span class="lnr lnr-thumbs-up"></span><br><br>'+certification3text;
...
function fimIntro(){
    ...
    fimMarketingContainer.className="fimMarketingContainer blink";
    fimMarketingContainer.innerHTML='<div id="fimMarketingIcon" class="fimMarketingIcon"><span class="lnr lnr-graduation-hat"></span></div>';
	...
	var fimMarketingCursosContainer = document.createElement('div');
	fimMarketingCursosContainer.setAttribute('id','fimMarketingCursosContainer');    
    fimMarketingCursosContainer.className="fimMarketingCursosContainer"

	var fimMarketingCurso1 = document.createElement('div');
    fimMarketingCurso1.setAttribute('id','fimMarketingCurso1');    
    fimMarketingCurso1.className="fimMarketingCurso";
    fimMarketingCurso1.innerHTML="<a onclick='window.open(this.href)' href='http://www.dominiosistemas.com.br/ctd_detalhes/94' target='_blank'><img src='https://moodle.dominiosistemas.com.br/assets/logos/Rotinas.png'>"+"<span>Rotinas do Departamento Pessoal</span>";
    ...
    fimMarketingCurso3.className="fimMarketingCurso"
    fimMarketingCurso3.innerHTML="<a onclick='window.open(this.href)' href='http://www.dominiosistemas.com.br/ctd_detalhes/88' target='_blank'><img src='https://moodle.dominiosistemas.com.br/assets/logos/conceitos_do_esocial.png'>"+"<span>Conceitos do eSocial</span>"

    $(fimMarketingContainer).bind(changeClick,function(e){
    	console.log('andre')
    	fimMarketingContainer.appendChild(fimMarketingIconText)
    	logoCourse.className+=" goUpBlur"
    	fimText.className="fimText goUpBlur";
    	//fimContainer.className+=" goUp";
    	fimMarketingIcon.className+=" fimMarketingIconBlue"
    	fimMarketingIconText.className+=" fimMarketingBlue"
    	fimMarketingBackground.appendChild(fimMarketingContainer);
		fimMarketingBackground.className+=" fimMarketingBackground goUp"
		fimMarketingContainer.className="fimMarketingContainerBlue";
		fimMarketingBackground.appendChild(fimMarketingCursosContainer);
		fimMarketingCursosContainer.appendChild(fimMarketingCurso1);
...
function chooseTopicAndContinue(){
	...
	//////////////////////////////////////007//////////////////////////////////////	
		...
		textoQuery.className="blurIn"
        }catch(e){}
		$(nomeTopicoText).html(cpInfoCurrentSlideLabel);
        logoCourse.className="logoCourseFvm blurIn"
        ...
        if(fvmIntroNull==true){
			...
			fvmIntro.setAttribute('id','fvmIntro')
			document.getElementById("project").appendChild(fvmIntro);
			fvmIntro.className="fvmIntro blurIn"
	 		
	 		var fvmIntroTitle = document.createElement("div");
			fvmIntro.appendChild(fvmIntroTitle);
			fvmIntroTitle.innerHTML=slideNotesTitle;
			fvmIntroTitle.className="painelTitleSlide"

			var fvmIntroText = document.createElement("div");
			$(fvmIntro).append(fvmIntroText);
			fvmIntroText.innerHTML=slideNotesText
			fvmIntroText.className="painelTextoSlide";
	...
	$(iniciaElement).bind(changeClick,function(e){
		...
		else if(cpInfoCurrentSlideLabel == "Fa̤a Voc̻ Mesmo!")
		{
			...
			nomeTopicoContainer.className="blurOut"
            avaliacaoPratica.className="avaliacaoPratica blurOut";
			videoIntroElement.className="blurOut"
            logoCourse.className="logoCourseFvm blurOut";
            try{
            	fvmIntro.className="blurOut";
           	}catch(e){}
            logoCourse.addEventListener("animationend", function(){
                cp.jumpToNextSlide();
			    fvm();    
...
function fvm(){
	...
	firstTrie.setAttribute("style", "position:absolute; z-index:100; font-size:2em;");
	...
	$(firstTrie).html('<div style="display:table-cell;vertical-align:middle;text-align:center;pointer-events:none;"><span class="lnr lnr-checkmark-circle"></span></div>');
	...
	secondTrie.setAttribute("style", "position:absolute; z-index:100;font-size:2em;");
	...
	$(secondTrie).html('<p style="display:table-cell;vertical-align:middle;text-align:center;pointer-events:none;"><span class="lnr lnr-checkmark-circle"></span></p>');
	...
	thirdTrie.setAttribute("style", "position:absolute; z-index:100; font-size:2em;");
	...
	$(thirdTrie).html('<p style="display:table-cell;vertical-align:middle;text-align:center;pointer-events:none;"><span class="lnr lnr-checkmark-circle"></span></p>');
	...
	pointerIcon.setAttribute("style", "position:absolute; z-index:100; font-size:2em;");
	...
	$(pointerIcon).html('<i class="fa fa-mouse-pointer" aria-hidden="true"></i>');

	var dicaBtn = document.createElement("div");
	dicaBtn.setAttribute("id", "dicaBtn");
	document.getElementById("project_main").appendChild(dicaBtn);
	dicaBtn.setAttribute("style", "position:absolute; z-index:100;width:50px;height:50px;display: flex; justify-content: center;align-items: center;");
	...
	$(dicaBtn).html('<div style="display: flex;justify-content: center;align-items: center;font-size: 2em;color: white;"><span class="lnr lnr-bubble"></span></div>');
	...
	function checkNumberOfTries()
	{
		...
		if(numberOfTries == 0 || numberOfTries == undefined)
		{
			...
			TweenLite.to(firstTrie,.25, {background:'#2186fa'});
			TweenLite.to(secondTrie,.25, {background:'#2186fa'});
			TweenLite.to(thirdTrie,.25, {background:'#2186fa'});
...
function createPainel(){
	...
	$(nextBtn).html('<p style="display:table-cell;vertical-align:middle;text-align:center;"><span class="big lnr lnr-chevron-right"></span></p>');
	...
	$(previousBtn).html('<p style="display:table-cell;vertical-align:middle;text-align:center;"><span class="big lnr lnr-chevron-left"></span></p>');
	...
	if(window.location.host=='dominiosistemas.webaula.com.br' || window.location.host=='moodle.dominiosistemas.com.br'){
		...
		$(project).append('<br><div id="ranger" class="ranger"><input type="range" min="1" max="'+(cpInfoSlideCount-2)+'" onchange="cpCmndGotoSlideAndResume=this.value" onKeyPress="" required="" autocomplete="off" placeholder="Digite a tela"></div>');
	}
...
function checkInteractionNext(){
	...
	var intervencaoText = document.querySelectorAll("input[id*=_inputField],div[class=cp-input]")
...
function createSidePainel(){
	...
	$(showSidePainel).html('<span class="lnr lnr-menu"></span>');
	...
	emailBtn.setAttribute("style", "z-index:100");
		...
		TweenLite.to(emailBtn,.3,{opacity:.5,onComplete:function()
			{
				...
				window.top.location = "mailto:"+cpinfoEmailTR+"?subject="; 
			}
		});
	});
	...
	soundBtn.setAttribute("style", "z-index:100");
	...
	$(soundBtn).html('<p style="display:table-cell;vertical-align:middle;text-align:center;pointer-events:none;"><span class="big lnr lnr-volume-high"></span><br>SOM</p>');
	sidePainel.appendChild(soundBtn);

	var painelBtn = document.createElement("div")
	painelBtn.setAttribute("id", "painelBtn");
	painelBtn.setAttribute("style", "z-index:150");
	...
	$(language).css("background-color",'#ff8000');
...
function beneficios(){
	...
	document.getElementById("painelTextoSlide").appendChild(logoCourseClone);
	logoCourseClone.setAttribute("id", "logoCourseClone");
	logoCourseClone.className="beneficiosCourseLogo ";
	...
	$(beneficioGroup).append(beneficio1);

	var beneficio2 = document.createElement("div");
	beneficio2.setAttribute("id", "beneficio2");
	$(beneficioGroup).append(beneficio2);

	var beneficio3 = document.createElement("div");
...
function translateBackground(){
	var elementClick = document.querySelector('div[id*=Click],div[id*=Text_Entry],canvas[id*=Mouse]');
	var elementSystemImage = document.getElementById("div_Slide")
	if(document.documentElement.clientWidth < 630 && elementClick != null)
	{
		//elementClick.style.border="solid black 20px";
		...
		//elementSystemImage.className="elementSystemImage"
		//$(elementSystemImage).css('transform-origin:','0px 0px');
		//$(elementSystemImage).css('transform','scale(.33)');	
		elementSystemImage.setAttribute("style", "transform:scale(0."+document.documentElement.clientWidth+"); transform-origin:left top");
...
function messageFailureToPanel(){
	if(document.documentElement.clientWidth < 420){
		var failureMessage = document.querySelectorAll('[id*=Failure_Caption] > p');
		if(failureMessage.length != 0){
			var failureCaption = failureMessage[0].innerHTML;
...
function updateSlideElements(){
	...
	//topicLanguage();
	//changeToDeviceFont();
	//console.log(cpInfoCurrentSlide)
	...
	if(cpInfoCurrentSlide == cpInfoSlideCount)			
	{
		...
		$(messageFinalElementIcon).css('display',"none");
		$(messageFinalElementText).html('<span class="big lnr lnr-sync"></span><br>'+msgFinalTopico);
