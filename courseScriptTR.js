...
var courseOptionsEn = '<h2>To access the course options,<br>access the icon on the bottom right corner.'
var courseOptionsPt = '<h2>Para acesso as opções do curso,<br>toque no canto inferior direito.'
var introType;
...
function courseName(){
	...
	console.log(courseName[4])
	if(courseName[1] == 'Honorários' && courseName[3] == 'Central')
	{
		courseName = 'honorariosCentral.png';
	}
...
function fimIntro(){
	...
	$(messageFinalElementText).html('<br><br><br>Parabéns! Você concluiu o curso <br><br><strong>'+cpInfoProjectName+'</strong><br><br>Dúvidas? Envie um e-mail para <br>'+cpinfoEmailTR+'');
...
function createSidePainel(){
	...
	$(painelBtn).html('<p style="display:table-cell;vertical-align:middle;text-align:center;pointer-events:none;"><span class="big lnr lnr-upload"></span><br>PAINEL</p>');
	...
	//$(painelBtn).bind("click", function(){
		if(painelMostra)
	  	...
	  	else 
	  	{
			...
			if(cpInfoCurrentSlideLabel == "Painel Lateral!" || cpInfoCurrentSlideLabel == "Beneficios!" ){
				//painel.className='moveIn'
	...
	$(emailBtn).html('<p style="display:table-cell;vertical-align:middle;text-align:center;pointer-events:none;"><span class="big lnr lnr-envelope"></span></i><br>E-MAIL</p>');
...
try{
	...
	if(successCaption[0].style.visibility = 'visible'){
		var resuccessCaption = $("[id*='re-Success_Caption_']")
		$(resuccessCaption).css('left',0);
		$(resuccessCaption).css('top',0);
		$(resuccessCaption).css('width','100%');
		$(resuccessCaption).css('height','100%');
		$(resuccessCaption).css("background-color","rgba(102, 102, 102, .8");
		$(resuccessCaption).html("<div class='successCaption'><span class='lnr lnr-thumbs-up';></span></div>");
	}else{}
	}catch(e){}
}
...
function painelLateral(){
	...
	if(document.documentElement.clientWidth < 420)
	{
			...
			painelTextoSlide.style.maxHeight=(painel.clientHeight-painelTitleSlide.clientHeight)-165+'px';
...
function beneficios(){
	...
	beneficioGroup.innerHTML="<span>"+nomeAluno+" veja os benefícios, menos trabalho para você, mais tempo e qualidade para atender os seus clientes."+"</span>";
...
function warningElement(message,startTime,endTime,top,left,angle){
	...
	document.getElementById("project_main").appendChild(warningElementText);
	...
	$(warningElementText).css('color',"white");
